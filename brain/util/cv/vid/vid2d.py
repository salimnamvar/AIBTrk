...
Classes:
    - `Video2D`: Represents a 2D video object for capturing video from files, image sequences, or cameras.
...
import cv2
from brain.util import BaseObject, Frame2D
...
class Video2D(BaseObject):
    ...
    def read(self) -> Tuple[bool, Frame2D]:
        ...
        if ret:
            ...
            frame = Frame2D(a_data=frame, a_video_id=self._id, a_sequence_id=frame_seq_id)
        return ret, frame
