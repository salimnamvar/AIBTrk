...
Classes:
    `Frame2D`:
        This class defines a 2D frame in the format of [Height, Width, Channels] as an `Image2D` that is from a
        `Video2D`.
...
import uuid
import numpy as np
from brain.util import Image2D
...
class Frame2D(Image2D):
    ...
    def __init__(self, a_data: np.ndarray, a_filename: str = None, a_sequence_id: int = None, a_id: uuid.UUID = None,
                 a_video_id: uuid.UUID = None, a_name: str = 'FRAME2D'):
        """Constructor
        ...
        if a_sequence_id is not None and not isinstance(a_sequence_id, int):
            raise TypeError(f"`a_sequence_id` argument must be an `int` but it's type is `{type(a_sequence_id)}`")
