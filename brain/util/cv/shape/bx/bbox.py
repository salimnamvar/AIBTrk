...
Classes:
    - `BBox2D`: Represents a 2D bounding box with an additional confidence score.
    - `BBox2DList`: A container for a collection of BBox2D objects.
...
import numpy as np
from brain.util import (Box2D, Point2D, Size, BBoxCoordStatus, BBoxConfidenceStatus, BBoxSizeStatus, is_float,
                        xywh_to_xyxy, cxyar_to_xyxy, Box2DList, BaseList)
...
class BBox2D(Box2D):
    ...
    def __init__(
        ...
        # Statuses
        self.coord_status: BBoxCoordStatus = BBoxCoordStatus.UNKNOWN
        self.conf_status: BBoxConfidenceStatus = BBoxConfidenceStatus.UNKNOWN
        self.size_status: BBoxSizeStatus = BBoxSizeStatus.UNKNOWN
    ...
    def _validate_dtypes(self, a_p1: Point2D, a_p2: Point2D, a_score: float) -> Tuple[Point2D, Point2D, float]:
    ...
    def _validate_xyxy(self, a_p1: Point2D, a_p2: Point2D) -> None:
        ...
        if a_p1.x > a_p2.x or a_p1.y > a_p2.y:
            if self.strict:
                raise ValueError("Incorrect BoundingBox2D format. It must be in type [x, y, x, y].")
            self.coord_status = BBoxCoordStatus.INVALID_COORDINATES
        elif a_p1 == a_p2:
            if self.strict:
                raise ValueError("Given top-left and bottom-right points must be distinct.")
            self.coord_status = BBoxCoordStatus.INVALID_COORDINATES
        elif (self.img_size is not None) and (
            (a_p2.x <= 0 and a_p2.y <= 0) or (a_p1.x >= self.img_size.width and a_p1.y >= self.img_size.height)
        ):
            if self.strict:
                raise ValueError("Given bounding box's points are entirely out of bounds.")
            self.coord_status = BBoxCoordStatus.ENTIRELY_OOB
        elif (self.img_size is not None) and (
            a_p1.x < 0 or a_p1.y < 0 or a_p2.x > self.img_size.width or a_p2.y > self.img_size.height
        ):
            if self.strict:
                raise ValueError("Given bounding box's points are partially out of bounds.")
            self.coord_status = BBoxCoordStatus.PARTIALLY_OOB
        elif self.img_size is not None:
            self.coord_status = BBoxCoordStatus.VALID
    ...
    def _validate_score(self, a_score: float):
        ...
        if self.conf_thre is not None and a_score < self.conf_thre:
            ...
            self.conf_status = BBoxConfidenceStatus.NOT_CONFIDENT
        elif self.conf_thre is not None:
            self.conf_status = BBoxConfidenceStatus.CONFIDENT
    ...
    def _validate_size(self, a_size: Size):
        ...
        if (self.min_size_thre is not None) and not (a_size >= self.min_size_thre):
            ...
            self.size_status = BBoxSizeStatus.INVALID
        elif self.min_size_thre is not None:
            self.size_status = BBoxSizeStatus.VALID
    ...
    @classmethod
    def from_xyxys(cls, a_coordinates: Union[Tuple, List, np.ndarray], **kwargs) -> "BBox2D":
    ...
    @classmethod
    def from_xywhs(cls, a_coordinates: Union[Tuple, List, np.ndarray], **kwargs) -> "BBox2D":
    ...
    @classmethod
    def from_cxyars(cls, a_coordinates: Union[Tuple, List, np.ndarray], **kwargs) -> "BBox2D":
    ...
    def clamp(self) -> None:
        ...
        """
        if (
                self.coord_status == BBoxCoordStatus.PARTIALLY_OOB or self.coord_status == BBoxCoordStatus.VALID
        ) and self.img_size is not None:
            p1 = Point2D(a_x=max(self.p1.x, 0), a_y=max(self.p1.y, 0))
            p2 = Point2D(a_x=min(self.p2.x, self.img_size.width), a_y=min(self.p2.y, self.img_size.height))
...
class BBox2DList(Box2DList, BaseList[BBox2D]):
    """BBox2D List
    ...
    def __init__(self, a_name: str = "BBox2DList", a_max_size: int = -1, a_items: List[BBox2D] = None):
    ...
    @classmethod
    def from_xyxys(cls, a_coordinates: Union[Tuple, List, np.ndarray], **kwargs) -> "BBox2DList":
        ...
        bboxes = BBox2DList()
        bboxes.append(a_item=[BBox2D.from_xyxys(coord, **kwargs) for coord in coordinates])
        return bboxes

    @classmethod
    def from_xywhs(cls, a_coordinates: Union[Tuple, List, np.ndarray], **kwargs) -> "BBox2DList":
        ...
        # Instantiate bounding boxes
        bboxes = BBox2DList()
        bboxes.append(a_item=[BBox2D.from_xywhs(coord, **kwargs) for coord in coordinates])
        return bboxes
    ...
    def remove(
        self,
        a_status: Union[
            BBoxCoordStatus, BBoxConfidenceStatus, BBoxSizeStatus, List[Union[BBoxCoordStatus, BBoxConfidenceStatus, BBoxSizeStatus]]
        ],
    ):
        """
        ...
        if not all(isinstance(status, (BBoxCoordStatus, BBoxConfidenceStatus, BBoxSizeStatus)) for status in a_status):
            raise TypeError(
        ...
        for index in reversed(range(len(self.items))):
            ...
            if any(
                (isinstance(status, BBoxCoordStatus) and item.coord_status == status)
                or (isinstance(status, BBoxConfidenceStatus) and item.conf_status == status)
                or (isinstance(status, BBoxSizeStatus) and item.size_status == status)
    ...
    def to_xyxys(self) -> np.ndarray:
        ...
        Returns:
            np.ndarray:
                ...
                list.
        """
        return np.vstack([box.to_xyxys() for box in self.items])
