...
import numpy as np
from brain.util import BaseObject, Point2D, Size, BaseList
...
class Box2D(BaseObject):
    ...
    def speed(self, a_box2d: "Box2D") -> Tuple[float, float]:
        """Calculates the normalized speed vector from the center of the current bounding box to another bounding box.
        ...
        if a_box2d is None or not isinstance(a_box2d, Box2D):
            raise TypeError("The `a_box2d` should be a `Box2D`.")
        speed_x, speed_y = self.center.speed(a_point2d=a_box2d.center)
...
class Box2DList(BaseList[Box2D]):
    """Box2D List
    ...
    def __init__(self, a_name: str = "Box2DList", a_max_size: int = -1, a_items: List[Box2D] = None):
