...
This module provides utility functions for converting bounding box coordinates between different formats and defines
enums for tracking the coordination, confidence, and size statuses of bounding boxes.

Classes:
    `BBoxCoordStatus`:
        Enum representing the coordination statuses of a bounding box.
    `BBoxConfidenceStatus`:
...
class BBoxCoordStatus(Enum):
    """BBox Coordination Status Enum
...
class BBoxConfidenceStatus(Enum):
